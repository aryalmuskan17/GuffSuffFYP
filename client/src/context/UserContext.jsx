...
const UserProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (auth.token) {
      ...
      const fetchUserProfile = async () => {
        try {
          // Note: The backend URL is hardcoded here (http://localhost:5001) but should ideally be in a .env file.
          const res = await axios.get('http://localhost:5001/api/auth/profile');
          // Persist user data on success
  ...
  }, [auth.token]); // The effect runs whenever the token changes.
